        ...
        <div className="flex flex-col lg:flex-row gap-12">
              ...
              <div className="bg-[#1f1f1f] p-6 rounded-lg space-y-4">
                ...
                <div className="flex flex-col gap-3">
                  {anime.externalLinks.map((link: any) => (
                    ...
                    >
                      {link.icon && (
                        <img src={link.icon} alt="" className="w-5 h-5 rounded-sm" />
                      )}
                      <span className="group-hover:underline">{link.site}</span>
                    </a>
                  ))}
                </div>
              </div>
          ...
          <div className="flex-1 pt-4">
            ...
            <div className="flex flex-wrap gap-2 mb-8 items-center">
              ...
              {anime.tags?.filter((t: any) => !t.isMediaSpoiler && t.rank >= 60).slice(0, 5).map((tag: any) => (
                 <span
                  key={tag.id}
                  className="text-sm text-gray-400 hover:text-white cursor-pointer transition mr-2"
                >
                  #{tag.name}
                </span>
              ))}
              ...
              <div className="mb-12">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 max-w-4xl">
                  {anime.characters.edges.map((char: any) => (
                    <div
                      key={char.node.id}
                      className="flex bg-[#1f1f1f] rounded-lg overflow-hidden"
                    >
                      <img
