...
export default async function Home() {
  let trendingAnime = [];
  try {
    const data = await fetchAniList(TRENDING_ANIME_QUERY, { page: 1, perPage: 12 });
  ...
  return (
    <main className="min-h-screen bg-gray-900 text-white p-8">
      <header className="max-w-7xl mx-auto flex justify-between items-center mb-12">
        ...
        <nav>
          <Link href="/search" className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg transition">
            Search Anime
          </Link>
        </nav>
      </header>
