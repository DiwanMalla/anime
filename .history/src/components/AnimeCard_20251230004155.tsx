          ...
          <div className="relative aspect-[2/3] w-full overflow-hidden">
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 scale-90 group-hover:scale-100">
              <div 
                ...
                style={{ background: `radial-gradient(circle, ${accentColor}dd, transparent)` }}
              >
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-white/5">
              <div className="flex items-center gap-1.5 text-gray-500">
                ...
                <span className="text-[10px] font-medium">{anime.seasonYear}</span>
              </div>
              <div className="flex items-center gap-1.5 text-gray-500">
                <Layers className="h-3 w-3" />
                <span className="text-[10px] font-medium">{anime.episodes || "?"} EP</span>
              </div>
              <div className={`text-[10px] font-bold uppercase tracking-tighter ${
                anime.status === "RELEASING" ? "text-blue-400" : 
                anime.status === "FINISHED" ? "text-green-400" : "text-purple-400"
              }`}>
                {anime.status === "RELEASING" ? "Airing" : 
                 anime.status === "FINISHED" ? "Done" : "Soon"}
              </div>
            </div>
          </div>
        </div>
